...
class RevAddOnFilmstripUpdate {
	...
	private $remote_url_info	= 'http://updates.themepunch.tools/addons/revslider-filmstrip-addon/revslider-filmstrip-addon.php';
	...
	protected function _check_updates() {
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				...
				//update_option('revslider_filmstrip_addon-stable-version', $data->full->stable);
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('revslider_filmstrip_addon-valid', 'false');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('revslider-filmstrip-addon-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider-filmstrip-addon-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('revslider-filmstrip-addon-update-check-short', time());
			...
			$response = wp_remote_post($this->remote_url, array(
				'user-agent' => 'WordPress/'.$wp_version.'; '.get_bloginfo('url'),
				'body' => array(
					'item' => urlencode('revslider-filmstrip-addon'),
					'version' => urlencode($this->version),
					'code' => urlencode($purchase)
				)
			));
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('revslider_filmstrip_addon-connection', false);
				return false;
			}else{
				update_option('revslider_filmstrip_addon-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('revslider_filmstrip_addon-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('revslider_filmstrip_addon-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('revslider_filmstrip_addon-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('revslider_filmstrip_addon-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('revslider_filmstrip_addon-valid', 'false');
					update_option('revslider_filmstrip_addon-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('revslider-filmstrip-addon-update-check', '');
		}
