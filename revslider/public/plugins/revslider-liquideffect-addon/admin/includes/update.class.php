...
class RevAddOnLiquidEffectUpdate {
	...
	private $remote_url_info	= 'http://updates.themepunch.tools/addons/revslider-liquideffect-addon/revslider-liquideffect-addon.php';
	private $plugin_slug		= 'revslider-liquideffect-addon';
	private $plugin_path		= 'revslider-liquideffect-addon/revslider-liquideffect-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates() {
		...
		$last_check = get_option('revslider_liquideffect_addon-update-check');


		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider_liquideffect_addon-update-check', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				update_option('revslider_liquideffect_addon-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
					
				//update_option('revslider_liquideffect_addon-stable-version', $data->full->stable);
				update_option('revslider_liquideffect_addon-latest-version', $data->full->version);
			}
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('revslider_liquideffect_addon-valid', 'false');
		$purchase = (get_option('revslider-valid', 'true') == 'true') ? get_option('revslider-code', '') : '';
		$rattr = array(
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('revslider-liquideffect-addon-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider-liquideffect-addon-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('revslider-liquideffect-addon-update-check-short', time());
			...
			$response = wp_remote_post($this->remote_url, array(
				'user-agent' => 'WordPress/'.$wp_version.'; '.get_bloginfo('url'),
				'body' => array(
					'item' => urlencode('revslider-liquideffect-addon'),
					'version' => urlencode($this->version),
					'code' => urlencode($purchase)
				)
			));
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('revslider_liquideffect_addon-connection', false);
				return false;
			}else{
				update_option('revslider_liquideffect_addon-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('revslider_liquideffect_addon-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('revslider_liquideffect_addon-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('revslider_liquideffect_addon-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('revslider_liquideffect_addon-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('revslider_liquideffect_addon-valid', 'false');
					update_option('revslider_liquideffect_addon-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('revslider-liquideffect-addon-update-check', '');
		}
