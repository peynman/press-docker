...
class RsAddOnExplodinglayersBase {
	...
	protected function loadClasses() {
		...
		if($isAdmin) {
			...
			// admin init
			new RsExplodinglayersSliderAdmin(static::$_PluginTitle, static::$_Version);
			new RsExplodinglayersSlideAdmin(static::$_PluginTitle, static::$_PluginPath);
			...
			switch($page) {
				...
				case 'revslider':
					...
					if(isset($_GET['view']) && $_GET['view'] === 'slide' && isset($_GET['id'])) {
						...
						wp_enqueue_script($_handle, $_base . 'js/' . static::$_PluginTitle . '-slide-admin.js', array('jquery', 'unite_settings', 'unite_admin', 'wp-color-picker', 'tp-color-picker-js'), static::$_Version, true);
