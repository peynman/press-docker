...
class RsAddOnDuotoneFiltersBase {
	...
	protected function systemsCheck() {
		...
		else if(!version_compare(RevSliderGlobals::SLIDER_REVISION, RsAddOnDuotoneFiltersBase::MINIMUM_VERSION, '>=')) {
		
			return 'add_notice_version';
		
		}
		else if(get_option('revslider-valid', 'true') == 'false') {
	...
	protected function loadClasses() {
		...
		if($isAdmin) {
			...
			$update_admin = new RevAddOnDuotoneFiltersUpdate(static::$_Version);
			...
			// admin init
			new RsDuotoneFiltersSliderAdmin(static::$_PluginTitle, static::$_Version);
			new RsDuotoneFiltersSlideAdmin(static::$_PluginTitle, static::$_PluginPath);
		...
		*/
		...
		new RsDuotoneFiltersSliderFront(static::$_Version, static::$_PluginUrl, static::$_PluginTitle, $isAdmin);
