...
class RevAddOnRefreshUpdate {
	...
	private $remote_url_info	= 'https://updates.themepunch.tools/addons/revslider-refresh-addon/revslider-refresh-addon.php';
	...
	protected function _check_updates() {
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				...
				//update_option('revslider_refresh_addon-stable-version', $data->full->stable);
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('revslider_refresh_addon-valid', 'false');
		$purchase = (get_option('revslider-valid', 'true') == 'true') ? get_option('revslider-code', '') : '';
		$rattr = array(
			'code' => urlencode($purchase),
			'version' => urlencode(REV_ADDON_REFRESH_VERSION)
		);
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('revslider-refresh-addon-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider-refresh-addon-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('revslider-refresh-addon-update-check-short', time());
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('revslider_refresh_addon-connection', false);
				return false;
			}else{
				update_option('revslider_refresh_addon-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('revslider_refresh_addon-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('revslider_refresh_addon-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('revslider_refresh_addon-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('revslider_refresh_addon-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('revslider_refresh_addon-valid', 'false');
					update_option('revslider_refresh_addon-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('revslider-refresh-addon-update-check', '');
		}
