...
class RevSliderAdmin extends RevSliderBaseAdmin{
	...
	private function init(){
        ...
        $jquery_plugin_temp_active = get_option('jquery-plugin-temp-active', 'false');
        if ($jquery_plugin_temp_active == 'true') {
            $temp_force = (isset($_GET['checktempactivate'])) ? true : false;
            $ci = &get_instance();
            $ci->load->library('updater');
            $ci->updater->add_temp_active_check($temp_force);
        }
    ...
    public static function plugin_action_links($links, $file){
        if ($file == plugin_basename(RS_PLUGIN_FILE_PATH)){
            $rs_enabled = get_option('revslider-valid', 'true');
	...
	public static function onAddScripts(){
		...
		wp_add_inline_style( 'rs-plugin-settings', '<style type="text/css" id="rs-inline-captions-css">'.$styles.'</style>');

		$custom_css = RevSliderOperations::getStaticCss();
		$custom_css = RevSliderCssParser::compress_css($custom_css);
		wp_add_inline_style( 'rs-plugin-settings', '<style type="text/css" id="rs-inline-static-css">'.$custom_css.'</style>');
	}
	...
	private static function importSliderOnlineTemplateHandleNew($data, $viewBack = null, $updateAnim = true, $updateStatic = true, $single_slide = false){
		...
		if($uid == ''){
			...
			$package = isset($data['package']) ? esc_attr($data['package']) : false;
		...
		try{
			...
			switch($action){
                ...
                case 'create_draft_page':
                    ...
                    if($pageUrl > 0){
						$response['open'] = get_permalink($pageUrl);
					}
                    self::ajaxResponseData($response);
				break;
